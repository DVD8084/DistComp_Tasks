		...
		for _, cmd := range list.Commands() {
			if cmd.HasUserCallback() {
				...
				case ImageTypeSimpleTexture:
					gl.ActiveTexture(gl.TEXTURE0 + uint32(0))
					gl.BindTexture(gl.TEXTURE_2D, uint32(textureID))
				case ImageTypeRGBATexture:
					gl.ActiveTexture(gl.TEXTURE0 + uint32(0))
					gl.BindTexture(gl.TEXTURE_2D, uint32(textureID))
...
func (renderer *OpenGL3) CreateFontsTexture() {
	// Build texture atlas
	io := imgui.CurrentIO()
	texture := io.Fonts().TextureDataAlpha8()
	...
	gl.TexImage2D(gl.TEXTURE_2D, 0, gl.RED, int32(texture.Width), int32(texture.Height),
		0, gl.RED, gl.UNSIGNED_BYTE, texture.Pixels)
...
func (renderer *OpenGL3) CreateImageTexture(path string) imgui.TextureID {
	...
	rgba := image.NewRGBA(texture.Bounds())
	draw.Draw(rgba, texture.Bounds(), texture, image.Point{0, 0}, draw.Src)
	...
	return TextureIDForRGBATexture(newTexture)
}
